	...
	C::Api: StatefulStorageRuntimeApi<Block>,
{
	fn get_paginated_storage(
		...
		let api_result = api.get_paginated_storage(self.client.info().best_hash, msa_id, schema_id);
	...
	fn get_itemized_storage(
		...
		let api = self.client.runtime_api();
		let api_result = api.get_itemized_storage(self.client.info().best_hash, msa_id, schema_id);
		map_result(api_result)
	}
}
