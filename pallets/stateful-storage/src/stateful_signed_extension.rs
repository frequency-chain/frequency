...
impl<T: Config> StatefulStorageValidate<T> for Pallet<T> {
	...
	fn validate_itemized(
		...
		Self::check_schema(*schema_id, PayloadLocation::Itemized, is_payload_signed, is_pruning)
	...
	fn validate_paginated(
		...
		Self::check_schema(*schema_id, PayloadLocation::Paginated, is_payload_signed, page_delete)
			.map_err(|e| map_dispatch_error(e))?;
