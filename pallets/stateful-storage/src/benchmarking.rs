...
benchmarks! {
	apply_item_actions {
		...
		for _ in 0..num_of_items {
			...
			assert_ok!(StatefulStoragePallet::<T>::apply_item_actions(RawOrigin::Signed(caller.clone()).into(), delegator_msa_id.into(), schema_id, actions));
		}
	...
	upsert_page {
		...
		assert_ok!(T::MsaBenchmarkHelper::add_key(provider_msa_id.into(), caller.clone()));
		assert_ok!(T::MsaBenchmarkHelper::set_delegation_relationship(provider_msa_id.into(), delegator_msa_id.into(), [schema_id].to_vec()));
	}: _(RawOrigin::Signed(caller), delegator_msa_id.into(), schema_id, page_id, payload.try_into().unwrap())
	verify {
