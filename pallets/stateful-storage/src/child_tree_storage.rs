...
impl ChildTreeStorage {
	...
	pub fn try_read<V: Decode + Sized>(
		...
		let value = child::get_raw(&child, &Identity::hash(key));
		match value {
			None => Ok(None),
	...
	pub fn prefix_iterator<V: Decode + Sized>(
		...
		let child = Self::get_child_tree(*msa_id);
		ChildTriePrefixIterator::<_>::with_prefix(&child, key)
	}

	/// Writes directly into child tree node
	pub fn write<V: Encode + Sized>(msa_id: &MessageSourceId, key: &[u8], new_value: V) {
	...
	pub fn kill(msa_id: &MessageSourceId, key: &[u8]) {
		let child_trie_info = &Self::get_child_tree(*msa_id);
