...
use common_primitives::{
    ...
    stateful_storage::{
        ItemizedStoragePageResponse, ItemizedStoragePageResponseV2, ItemizedStorageResponseV2, PageHash, PageId,
        PaginatedStorageResponse, PaginatedStorageResponseV2,
    },
};
...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn apply_item_actions(
            ...
            let intent = T::SchemaProvider::get_schema_info_by_id(schema_id).ok_or(Error::<T>::InvalidSchemaId)?;
            let actions: Vec<ItemActionV2<T::MaxItemizedBlobSizeBytes>> = actions.into_iter().map(|a| (schema_id, a).into()).collect();
        ...
        pub fn upsert_page(
            ...
            let schema = T::SchemaProvider::get_schema_info_by_id(schema_id).ok_or(Error::<T>::InvalidSchemaId)?;
            let caller_msa_id =
        ...
        pub fn delete_page(
            ...
            let schema = T::SchemaProvider::get_schema_by_id(schema_id).ok_or(Error::<T>::InvalidSchemaId)?;
            Self::delete_page_v2(
                origin,
                state_owner_msa_id,
                schema.intent_id,
        ...
        pub fn apply_item_actions_with_signature_v2(
            ...
            let actions: Vec<ItemActionV2<T::MaxItemizedBlobSizeBytes>> = payload.actions.into_iter().map(|a| (payload.schema_id, a).into()).collect();
        ...
        pub fn apply_item_actions_v2(
            ...
            let caller_is_state_owner = caller_msa_id == state_owner_msa_id;
            Self::check_intent_for_write(intent_id,
                                         PayloadLocation::Itemized,
                                         caller_is_state_owner,
                                         is_pruning,
            )?;
        ...
        pub fn apply_item_actions_with_signature_v3(
            ...
            let is_pruning = payload.actions.iter().any(|a| matches!(a, ItemActionV2::Delete { .. }));
            Self::check_payload_expiration(
                frame_system::Pallet::<T>::block_number(),
                payload.expiration,
            )?;
...
impl<T: Config> Pallet<T> {
    ...
    pub fn get_paginated_storage_v1(
        ...
        Self::get_paginated_storage(msa_id, schema.intent_id).map(|v| v.into_iter().map(|p| <PaginatedStorageResponseV2 as Into<PaginatedStorageResponse>>::into(p)).collect())
    }
    ...
    pub fn get_itemized_storage(
        ...
        let items: Vec<ItemizedStorageResponseV2> = ItemizedOperations::<T>::try_parse(&page, false)
            .map_err(|_| Error::<T>::CorruptedState)?
            .items
            .iter()
            .map(|(key, v)| ItemizedStorageResponseV2::new(*key, match v.header {
            ...
            .collect();
        Ok(ItemizedStoragePageResponseV2::new(msa_id, intent_id, page.get_hash(), page.nonce, items))
    }
    ...
    fn check_intent_for_read(
        ...
        let intent = T::SchemaProvider::get_intent_by_id(intent_id)
            .ok_or(Error::<T>::InvalidIntentId)?;
