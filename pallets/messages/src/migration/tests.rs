...
use frame_support::{pallet_prelude::StorageVersion, traits::OnRuntimeUpgrade, BoundedVec};
...
fn lazy_migration_works() {
	new_test_ext().execute_with(|| {
		...
		let old_message =
			v2::Message { payload: payload.clone(), provider_msa_id: 1u64, msa_id: Some(1u64) };
		...
		let limit = <T as pallet_migrations::Config>::WeightInfo::progress_mbms_none() +
			...
			<weights::SubstrateWeight<T> as crate::weights::WeightInfo>::v2_to_v3_step() *
