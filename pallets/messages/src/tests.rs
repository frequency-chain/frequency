use super::mock::*;
use crate::{BlockMessages, Config, Error, Messages};
use common_primitives::messages::{BlockPaginationRequest, Message, SchemaId};
...
fn populate_messages(schema_id: SchemaId, message_per_block: Vec<u32>) {
	...
	for (idx, count) in message_per_block.iter().enumerate() {
		...
		for _ in 0..*count {
			list.push(Message { msa_id: 0, data: payload.clone(), index: counter, signer: 0 });
			counter += 1;
		}
...
fn add_message_should_store_message_on_temp_storage() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			list[0],
			(
				Message { msa_id: 0, data: message_payload_1.clone(), index: 0, signer: caller_1 },
				schema_id_1
			)
		);
		...
		assert_eq!(
			list[1],
			(
				Message { msa_id: 0, data: message_payload_2.clone(), index: 1, signer: caller_2 },
				schema_id_2
			)
		);
	});
