...
use crate::{storage::Storage, Permission};
use codec::Encode;
use frame_support::{
...
fn follow3_unfollow3() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(GraphPallet::follow3(Origin::signed(account_id_origin), 1234, 3));
		assert_ok!(GraphPallet::follow3(Origin::signed(account_id_origin), 1234, 1240));
		assert_ok!(GraphPallet::follow3(Origin::signed(account_id_origin), 1234, 2767378));

		assert_ok!(GraphPallet::follow3(Origin::signed(account_id_origin), 1240, 3));
		...
		assert_ok!(GraphPallet::unfollow3(Origin::signed(account_id_origin), 1234, 3));
		let perm = GraphPallet::read_from_child_tree(1234, GraphPallet::get_storage_key(3));
		assert_eq!(perm, None);
	});
}
