...
pub mod module {
	...
	impl<T: Config> Pallet<T> {
		...
		///
		#[pallet::call_index(4)]
		#[pallet::weight(T::WeightInfo::schedule_transfer().saturating_add(T::WeightInfo::transfer()))]
		...
		pub fn execute_scheduled_transfer(
			...
			Self::finalize_vesting_transfer_from_hold(&from, &to, schedule.clone())?;
...
impl<T: Config> Pallet<T> {
	...
	fn transfer_from_hold_balance(
		...
		// Exact - Will fail if the exact amount cannot be transfered
		// Free - deposit into the free balance but will locked following
		// Polite - Execute with regular previlages it consideres existential
	...
	fn finalize_vesting_transfer_from_free_balance(
		from: &T::AccountId,
		to: &T::AccountId,
		schedule: ReleaseScheduleOf<T>,
	) -> DispatchResult {
		Self::finalize_vesting_transfer(from, to, schedule, Self::transfer_from_free_balance)
	}

	fn finalize_vesting_transfer_from_hold(
		from: &T::AccountId,
		to: &T::AccountId,
		schedule: ReleaseScheduleOf<T>,
	) -> DispatchResult {
		Self::finalize_vesting_transfer(from, to, schedule, Self::transfer_from_hold_balance)
	}
}
