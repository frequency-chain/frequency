...
fn test_strip_unicode_whitespace() {
	...
	let string_with_whitespace = format!("{}hello{}world!{}", whitespace_string, whitespace_string, whitespace_string);

	let handle_converter = HandleConverter::new();
	let whitespace_stripped_string = handle_converter.strip_unicode_whitespace(&string_with_whitespace);
