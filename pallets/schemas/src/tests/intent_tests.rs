...
fn create_intent_via_governance_with_append_only_setting_and_non_itemized_should_fail() {
    new_test_ext().execute_with(|| {
        ...
        for location in
            [PayloadLocation::OnChain, PayloadLocation::IPFS, PayloadLocation::Paginated, PayloadLocation::OffChain]
        {
            // act and assert
            assert_noop!(
...
#[test]
fn create_intent_via_governance_with_pre_existing_name_should_fail() {
    new_test_ext().execute_with(|| {
        // arrange
        let sender: AccountId = test_public(1);
        let name = "namespace.descriptor";
