...
fn do_retarget_happy_path() {
	new_test_ext().execute_with(|| {
		...
		let staking_type = StakingType::FlexibleBoost;
...
fn do_retarget_flip_flop() {
	new_test_ext().execute_with(|| {
		...
		let to_amount = 300u64;
		let to_msa: MessageSourceId = 2;
		setup_provider(&staker, &from_msa, &from_amount, StakingType::FlexibleBoost);
		setup_provider(&staker, &to_msa, &to_amount, StakingType::FlexibleBoost);
...
fn check_retarget_multiple_stakers() {
	new_test_ext().execute_with(|| {
		...
		setup_provider(&staker_10k, &from_msa, &647u64, StakingType::FlexibleBoost);
		setup_provider(&staker_500, &to_msa, &293u64, StakingType::FlexibleBoost);
...
fn change_staking_target_errors_if_too_many_changes_before_thaw() {
	new_test_ext().execute_with(|| {
		...
		setup_provider(&staker, &from_msa, &staking_amount, StakingType::FlexibleBoost);
		setup_provider(&staker, &to_msa, &10u64, StakingType::FlexibleBoost);
...
fn change_staking_target_garbage_collects_thawed_chunks() {
	new_test_ext().execute_with(|| {
		...
		setup_provider(&staking_account, &from_target, &staked_amount, StakingType::FlexibleBoost);
		setup_provider(&staking_account, &to_target, &staked_amount, StakingType::FlexibleBoost);
...
fn change_staking_target_test_parametric_validity() {
	new_test_ext().execute_with(|| {
		...
		StakingAccountLedger::<Test>::insert(
			from_account,
			StakingDetails { active: 20, staking_type: StakingType::FlexibleBoost },
		);
		...
		let from_target: MessageSourceId = 3;
		let to_target: MessageSourceId = 4;
		setup_provider(&from_account, &from_target_not_staked, &0u64, StakingType::FlexibleBoost);
		setup_provider(&from_account, &from_target, &staked_amount, StakingType::FlexibleBoost);
		setup_provider(&from_account, &to_target, &staked_amount, StakingType::FlexibleBoost);
