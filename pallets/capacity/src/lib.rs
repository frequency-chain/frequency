...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// This adds a chunk to `StakingAccountDetails.stake_change_unlocking chunks`, up to `T::MaxUnlockingChunks`.  If `amount` is None, ALL Capacity targeted to `from` is
		/// reassigned.  If `amount` is Some(x), then the Capacity generated by `x`
		//  token originally targeted to the `from` MSA Id is reassigned to the `to` MSA Id
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_retarget(staker: &T::AccountId, from_msa: &MessageSourceId, to_msa: &MessageSourceId, amount: &Option<BalanceOf<T>>) -> Result<(), DispatchError> {
		...
		// do the update inside a try_mutate_exists

		// get or create the new staking target details
		// let mut to_details: etails<T> = Self::get_target_for(&staker, &to_msa).ok_or_default();
		//
		Ok(())
	}
}
