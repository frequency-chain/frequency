...
-   Every intent may also be optionally scoped to a namespace: `namespace.intent`


### 👤 Ownership and Control

Namespaces are currently envisioned to be owned by MSAs (Message Source Accounts), but there is an open question as to whether this ownership should instead be represented by a raw account (i.e., a public key). This decision will impact how access is managed, especially in governance and delegation flows.
...
-   Assign trusted publishers or revoke their rights (future work)
...
Delegation is the mechanism by which a user authorizes a provider to act on their behalf. Currently, this is limited to individual `SchemaId`s, but we propose expanding the delegation model to include:

-   Delegation by `SchemaId` (legacy)

-   Delegation by `ProtocolId` (implies all schema versions)
...
pub type Delegations<T> = StorageDoubleMap<
    ...
    Blake2_128Concat, MsaId,              // Provider
...
We explore three models for intent delegation:
...
-   Delegations may scope usage to specific namespaces (e.g., DSNP only)
...
### 🚫 Considered and Dismissed: Hybrid 2 — Global or Namespaced Intents
...
This mapping supports the hybrid delegation model (see Section 6) by ensuring that intent-level delegation can be resolved from a `SchemaId`.
...
-   Intents are only publishable by governance in the current model.

-   Future enhancements may allow namespaces to register private intents (if scoped delegation models are reconsidered).
...
The resolution logic depends on the type of delegation being supported:
...
```rust
pub type NamespaceOwner<T> = StorageMap<
    _, Blake2_128Concat, NamespaceId, MsaId
>;
...
We intentionally avoid maintaining a reverse map of `IntentId → SchemaId[]` to reduce runtime storage overhead, since this is primarily used for SDK tooling.

----------

### 🧹 Design Simplification

The current runtime includes a top-level flag indicating whether any delegation exists between a user and provider. This was used to shortcut delegation checks. In this model, that summary flag is dropped — its only purpose was to optimize for the empty case, which is already handled efficiently with the `Delegations` structure.

This simplification removes unnecessary complexity and keeps delegation logic self-contained.
...
-   There is no enforcement of namespace or protocol ownership.
...
Minor versions via namespace owner
...
Published by namespace owner under protocol
...
We may support a delegation model that allows namespace owners to appoint trusted publisher MSAs to publish new schema versions or protocols on their behalf.
...
-   Changing the list of implemented intents = Major
...
-   Should delegation resolution allow both **protocol** and **intent** types simultaneously, or require mutual exclusivity?
