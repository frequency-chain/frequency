var sourcesIndex = {};
sourcesIndex["common_helpers"] = {"name":"","files":["avro.rs","lib.rs","rpc.rs","types.rs"]};
sourcesIndex["common_primitives"] = {"name":"","dirs":[{"name":"parquet","files":["base.rs","column.rs","column_compression_codec.rs","numeric.rs","string.rs","temporal.rs","types.rs"]}],"files":["lib.rs","messages.rs","msa.rs","parquet.rs","schema.rs","utils.rs"]};
sourcesIndex["frequency"] = {"name":"","files":["main.rs"]};
sourcesIndex["frequency_cli"] = {"name":"","files":["cli.rs","command.rs","lib.rs"]};
sourcesIndex["frequency_runtime"] = {"name":"","dirs":[{"name":"weights","files":["block_weights.rs","extrinsic_weights.rs","mod.rs","paritydb_weights.rs","rocksdb_weights.rs"]}],"files":["lib.rs","xcm_config.rs"]};
sourcesIndex["frequency_service"] = {"name":"","dirs":[{"name":"chain_spec","files":["frequency_local.rs","mod.rs"]}],"files":["lib.rs","rpc.rs","service.rs"]};
sourcesIndex["pallet_messages"] = {"name":"","files":["lib.rs","types.rs","weights.rs"]};
sourcesIndex["pallet_messages_rpc"] = {"name":"","files":["lib.rs"]};
sourcesIndex["pallet_messages_runtime_api"] = {"name":"","files":["lib.rs"]};
sourcesIndex["pallet_msa"] = {"name":"","files":["lib.rs","types.rs","weights.rs"]};
sourcesIndex["pallet_msa_rpc"] = {"name":"","files":["lib.rs"]};
sourcesIndex["pallet_msa_runtime_api"] = {"name":"","files":["lib.rs"]};
sourcesIndex["pallet_schemas"] = {"name":"","files":["lib.rs","serde.rs","types.rs","weights.rs"]};
sourcesIndex["pallet_schemas_rpc"] = {"name":"","files":["lib.rs"]};
sourcesIndex["pallet_schemas_runtime_api"] = {"name":"","files":["lib.rs"]};
createSourceSidebar();
